({
    setColumns: function (component, isEditable) {
    ...
    doInitHelper: function (component, event, helper) {
        ...
        helper.checkIsEditable(component, event, helper);
    },
        ...
        action.setCallback(this, function (response) {
            ...
            if (state === 'SUCCESS') {
                ...
                if (sum > 100 && component.get('v.showToast')) {
                    helper.showToast('warning', 'Total contribution is more than 100.');
                }
    ...
    checkIsEditable : function(component, event, helper) {
        let action = component.get('c.isContributionEditable');
        let recordId = component.get('v.recordId');
        let params = {
            inventionDisclosureId: recordId
        };
        ...
        action.setCallback(this, function (response) {
            ...
            if (state === 'SUCCESS') {
                let result = response.getReturnValue();
                helper.setColumns(component, result);
            } else {
                let errors = response.getError();
                helper.showApexError(helper, errors);
            }
