({
    ...
    getSelectedInventorsHelper: function(component, event, helper) {
            action = component.get('c.getSelectedInventors');
            action.setParams({
                inventionDisclosureId: component.get('v.recordId'),
                decisionType : component.get('v.type'),
                vendorId: component.get('v.vendorId'),
                isTo: component.get('v.showTitle')
            });
        action.setCallback(
            this,
            $A.getCallback(
    ...
    getAdditionalInventors: function(component, helper, arrSelectedInvIds) {
        let action = component.get('c.getAdditionallySelectedInventors');
        action.setParams({
            inventorsIDs: arrSelectedInvIds
        });
        action.setCallback(
            this,
            $A.getCallback(
                function(response) {
                    ...
                    if (state === "SUCCESS") {
                        ...
                        helper.getSelectedInventorsToFlowHelper(component, arrSelectedInvIds);
                    } else if (state === "ERROR") {
                        let errors = response.getError();
                        console.error(errors);
                    }
