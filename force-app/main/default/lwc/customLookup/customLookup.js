...
export default class CustomLookup extends NavigationMixin(LightningElement) {
    ...
    get selection() {
        return this._curSelection.map((element) =>{
            let res = {...element};
            res.newTitle = res.subtitle ? res.title + '(' + res.subtitle + ')' : res.title;
            return res;
        });
    }
    ...
    processSelectionUpdate(isUserInteraction) {
        ...
        if (isUserInteraction) {
            const selectedIds = this._curSelection.map((sel) => sel.id);
            this.selectedTitles = this._curSelection.map((sel) => sel.title);
            console.log('selectedIds' , selectedIds)
            console.log('selectedTitles' , this.selectedTitles)
            this.dispatchEvent(new CustomEvent('selectionchange', { detail: {
                    selectedIds,
                    selectedTitles: this.selectedTitles
                }}));
            this.dispatchEvent(new CustomEvent('secondselectionchange', { detail: {
                    selectedIds,
                    selectedTitles: this.selectedTitles
                }}));
        }
