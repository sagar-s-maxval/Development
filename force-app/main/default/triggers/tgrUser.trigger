    ...
    if (trigger.isAfter)
    {
        try
        {
            ...
            {        
                ...
                {
                    list<SymphonyIPM__Inventor__c> lstInvs =[SELECT Id,Name,SymphonyIPM__User__c FROM SymphonyIPM__Inventor__c WHERE SymphonyIPM__User__c IN: ReactivatedUsrIds AND (RecordType.Name ='Platform Inventor' OR RecordType.Name ='Inventor')];
                    if(lstInvs.size()>0)
                    {
